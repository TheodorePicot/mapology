<div x-show="open" class="absolute w-[calc(100%-2rem)] top-20 left-0 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 mx-4"
    ...
    @auth
        <div class="w-full flex divide-y flex-col p-3">
            ...
            <div class="pt-3 w-full text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownUserAvatarButton">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="block rounded-md w-full px-4 text-start py-2 text-sm text-gray-700 dark:text-gray-200 hover:text-red-700 hover:font-semibold hover:bg-red-200 dark:hover:text-red-100 dark:hover:bg-red-600">Logout</button>
                </form>
            </div>
        </div>
    @else
